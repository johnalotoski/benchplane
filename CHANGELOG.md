...
- Add an explicit, package-owned single-device NVIDIA/CUDA llama.cpp target with a pre-initialization NVIDIA 610.43.03 host-driver floor, verified bounded device/offload provenance, target-aware comparison, and original experiment-to-resolved-plan verification while preserving the CPU default and evidence-v1 compatibility.
