...
The first cloud milestone may later extend this path to one single-GPU vLLM experiment, but AWS and vLLM execution remain intentionally unimplemented. Ordinary CI builds and semantically tests the CUDA package/evidence path without claiming real CUDA execution. The hardware path was additionally exercised on an NVIDIA GeForce RTX 4060 Laptop GPU with open driver 610.43.03 (CUDA driver API 13.3): the packaged CUDA 12.9 helper generated tokens, reported all 31 model layers offloaded, published two verified bundles, compared those GPU runs as compatible, and rejected a missing-device run without CPU fallback. This proves the bounded path on that configuration, not general NVIDIA compatibility or representative performance.
