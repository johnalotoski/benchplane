...
`experiment.yaml` is a required, checksum-covered payload, but the verifier does not currently parse it independently and prove that it resolves to `resolved-plan.json`. Semantic plan verification instead re-resolves the typed experiment embedded in `resolved-plan.json` and checks its deterministic identities. Original-input-to-resolved-plan semantic linkage remains intentionally deferred.
...
Checksum creation and verification hash payloads through bounded buffers rather than retaining whole-bundle bytes. Verification bounds checksum lines, inventory entries, provenance, resources, the typed resolved plan, and measurement file/line sizes; measurement JSONL is parsed and re-hashed incrementally, with only the size-bounded typed records retained for semantic verification and comparison. The verifier recomputes the repository's existing deterministic experiment and resolved-plan identities from the typed plan, then checks runtime/generator, phase/repetition order, counts, positive aggregate metrics, and the applicable request-observation contract. It also recomputes the existing `summary.json` latency mean/p50/p95 and mean-throughput values from measured repetition records using floor means and nearest-rank percentiles; a re-checksummed numerical contradiction is invalid evidence. It accepts the known historical aggregate-only llama v1 lineage and the current observation-bearing v2 lineage, rejects unknown combinations, and links optional provenance/resources to the resolved runtime when those backward-compatible extensions are present. Historical evidence-v1 without provenance/resources remains valid. It also rejects malformed or duplicate checksums, missing or extra payloads, absolute or traversing paths, symlinks and canonical-path escapes, non-regular files, unsupported formats, invalid or inconsistent run/attempt identities, malformed typed payloads, and empty required manifest fields.

## Descriptive comparison

`benchplane evidence compare BASELINE_BUNDLE CANDIDATE_BUNDLE [--json]` applies that complete verification path independently to both inputs before analysis and then requires different verified run IDs. Distinct IDs prevent self-comparison but do not prove statistical independence. The initial comparison contract accepts only successful, valid `local`/`llamaCpp` evidence using `benchplane-llama-cpp-smollm2/v2`. It requires equal generator, fixed model identity and digest, engine version, backend/device class, workload profile, output tokens, requests, concurrency, warmups, and measured repetitions. It deliberately does not require whole resolved-plan-digest equality because metadata, lifecycle, artifact, and budget fields do not change successful measurement meaning. Historical aggregate-only v1 remains verifiable but is not eligible for comparison.

For each bundle, comparison excludes warmups and computes request latency/TTFT count, floor mean, and nearest-rank p50/p95 from raw request observations. It separately computes measured-repetition aggregate latency/TTFT count, floor mean, nearest-rank p50/p95, and floor mean throughput from aggregate records. Baseline-to-candidate output includes signed absolute deltas and percentage deltas in thousandths of one percent; a percentage is absent when its baseline is zero. Throughput is not reconstructed from request timings because its repetition wall interval includes sequencing and cleanup outside individual request durations.

When present, resource deltas retain scope `helperProcessLifetime`: they include helper startup, model loading, warmups, measured work, and teardown. They are not divided by requests or tokens. Recorded architecture, CPU, OS/kernel, Benchplane/package, and Nix-store lineage similarities, differences, and unknowns are emitted separately as environment context and do not affect workload compatibility or imply causation. Comparison creates no evidence artifact and makes no confidence, significance, independence, causal, utilization, energy, or cross-host-equivalence claim.
