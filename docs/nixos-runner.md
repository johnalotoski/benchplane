...
The Benchplane NixOS module runs the existing public `benchplane run EXPERIMENT` command inside a systemd oneshot. It exercises all three local execution implementations, including the package-owned CPU-probe and llama.cpp children and the fixed Nix-store model; it is not a daemon or a controller-to-runner transport.
...
| `services.benchplane.runner.package` | Required package containing `bin/benchplane`, both sibling helpers, CPU-only llama.cpp, and the fixed GGUF model; the unit uses its absolute Nix store path. |
...
These controls do not define a fully isolated sandbox. The module does not add a network namespace, although the current local runtimes need no network and the fixed inference helper accepts no network input. It permits only Benchplane's package-owned children and read-only Nix-store model for this slice; broader device policy remains available for later concrete runtime composition. The current runner defines no controller protocol, provider abstraction, RPC service, queue, retry state, upload destination, credential map, hook, or plugin interface.
