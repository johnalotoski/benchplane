...
This composes the host NVIDIA driver with the existing runner, adds its unprivileged user to `video` and `render`, and changes the oneshot's device policy to allow only `/dev/nvidia0`, `/dev/nvidiactl`, and the NVIDIA UVM nodes. The pinned CUDA 12.9 Update 1 build contains code for compute capabilities 7.5, 8.0, 8.6, 8.9, 9.0, 10.0, 10.3, 12.0, and 12.1 and requires host driver 575.57.08 or newer. That is the conservative supported native-stack floor, not a claim of the older 525-series CUDA minor-version-compatibility mode or of every GPU/driver combination above it. The experiment still cannot choose another GPU or set `CUDA_VISIBLE_DEVICES`; Benchplane uses logical CUDA device 0 with fixed single-device full-model offload. The mutable kernel driver and device interface are outside the Nix closure. Do not enable this module for the `aarch64-linux` CPU package or without the runner.
