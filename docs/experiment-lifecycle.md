...
The packaged llama.cpp path follows the same successful transition sequence and complete-protocol/accounting rule. Its stable runtime failures distinguish `llamaCpp.spawnFailed`, `llamaCpp.modelInitFailed`, `llamaCpp.outputInvalid`, `llamaCpp.exitFailed`, `llamaCpp.deadlineExceeded`, and `llamaCpp.resourceAccountingFailed`; CPU probe analogously uses `cpuProbe.resourceAccountingFailed`. Each produces a failed attempt/run and indeterminate validity after allocation, then proceeds through evidence-v1 finalization when storage remains operational. An accounting issue never masks a primary nonzero/protocol/deadline failure. A later publication failure remains separate and never rewrites an already successful attempt.
