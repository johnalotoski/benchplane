...
Attempt status has the smaller `created → preparing → running → succeeded | failed | interrupted` model. During `preparing`, after the attempt snapshot exists and before the transition to `running`, Benchplane captures and persists bounded attempt-scoped execution provenance. An unavailable nonessential host fact is recorded as absent; a provenance persistence failure follows the existing allocated I/O failure path and does not create a runtime failure category. For helper-backed execution, concrete execution returns with exact process-lifetime accounting obtained at the same reap boundary, and that optional attempt-owned record is persisted after the attempt becomes terminal. Run-level `collecting` and `finalizing` therefore occur only after the attempt is terminal; an evidence-publication failure does not retroactively change a successfully executed attempt. This preserves the distinct dimensions of attempt provenance/resources and outcome, run outcome, measurement validity, and evidence publication outcome.
...
The packaged llama.cpp path follows the same successful transition sequence and complete-protocol/accounting rule. A successful protocol contains every repetition aggregate and every bounded request observation expected from the resolved warmup, repetition, and request counts. Missing, extra, out-of-order, or invalid request observations use the existing `llamaCpp.outputInvalid` failure; the complete parsed aggregate-and-observation prefix is discarded. Its other stable runtime failures remain `llamaCpp.spawnFailed`, `llamaCpp.modelInitFailed`, `llamaCpp.exitFailed`, `llamaCpp.deadlineExceeded`, and `llamaCpp.resourceAccountingFailed`; CPU probe analogously uses `cpuProbe.resourceAccountingFailed`. Each produces a failed attempt/run and indeterminate validity after allocation, then proceeds through evidence-v1 finalization when storage remains operational. Exact child resources may still survive applicable primary failures. An accounting issue never masks a primary nonzero/protocol/deadline failure. A later publication failure remains separate and never rewrites an already successful attempt.
