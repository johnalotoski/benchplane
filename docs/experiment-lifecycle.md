...
Attempt status has the smaller `created → preparing → running → succeeded | failed | interrupted` model. During `preparing`, after the attempt snapshot exists and before the transition to `running`, Benchplane captures and persists bounded attempt-scoped host and immutable-software provenance. An unavailable nonessential host fact is recorded as absent; a provenance persistence failure follows the existing allocated I/O failure path and does not create a runtime failure category. The NVIDIA helper can observe device/driver/offload facts only while initializing the CUDA runtime and model, so a successful protocol enriches that same attempt provenance during `running` before terminality; it adds no lifecycle state. For helper-backed execution, concrete execution returns with exact process-lifetime accounting obtained at the same reap boundary, and that optional attempt-owned record is persisted after the attempt becomes terminal. Run-level `collecting` and `finalizing` therefore occur only after the attempt is terminal; an evidence-publication failure does not retroactively change a successfully executed attempt. This preserves the distinct dimensions of attempt provenance/resources and outcome, run outcome, measurement validity, and evidence publication outcome.
...
A successful CPU probe follows `created → preparing → running → collecting → finalizing → succeeded`. Spawn failure, malformed or excessive output, nonzero child exit, runtime deadline, or missing mandatory accounting after an otherwise successful child makes the attempt and run `failed`, validity `indeterminate`, and CLI status `4`; normal evidence finalization and publication still proceed when storage works. CPU-probe output is accepted only after the complete child protocol and accounting succeed. If the child fails after emitting a valid prefix, that prefix is discarded. Nonzero/deadline failures preserve their primary code and may retain exact process resource evidence obtained at reap; spawn failure cannot.

The packaged llama.cpp path follows the same successful transition sequence and complete-protocol/accounting rule. A successful protocol contains every repetition aggregate and every bounded request observation expected from the resolved warmup, repetition, and request counts. The NVIDIA target additionally requires exactly one bounded device/offload metadata preamble consistent with its fixed policy. Missing, extra, out-of-order, or invalid output uses the existing `llamaCpp.outputInvalid` failure; the complete parsed aggregate-and-observation prefix is discarded. Its other stable runtime failures remain `llamaCpp.spawnFailed`, `llamaCpp.modelInitFailed`, `llamaCpp.exitFailed`, `llamaCpp.deadlineExceeded`, and `llamaCpp.resourceAccountingFailed`; a missing, malformed, or older-than-575.57.08 NVIDIA host driver, unavailable CUDA device, or incomplete model offload is a model-initialization failure after allocation. CPU probe analogously uses `cpuProbe.resourceAccountingFailed`. Each produces a failed attempt/run and indeterminate validity after allocation, then proceeds through evidence-v1 finalization when storage remains operational. Exact child resources may still survive applicable primary failures. An accounting issue never masks a primary nonzero/protocol/deadline failure. A later publication failure remains separate and never rewrites an already successful attempt.
